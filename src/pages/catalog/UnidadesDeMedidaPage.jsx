...
import { Plus, Ruler, Search, RefreshCw, Pencil, Trash2 } from 'lucide-react'
import {
  getUnidadesDeMedida,
  createUnidadDeMedida,
  updateUnidadDeMedida,
  deleteUnidadDeMedida,
} from '../../api/unidadesDeMedida'
...
// ─── Form (create / edit) ─────────────────────────────────────────────────────
function UnidadDeMedidaForm({ initial, onSuccess, onCancel }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (isEdit) {
        await updateUnidadDeMedida(initial.id, data)
      } else {
        await createUnidadDeMedida(data)
      }
      onSuccess()
    } catch (err) {
      ...
      if (fieldErrors) {
        ...
        setApiError(first ?? msg ?? 'Error al guardar la unidad de medida.')
      } else {
        setApiError(msg ?? 'Error al guardar la unidad de medida.')
      }
    }
  }
...
export default function UnidadesDeMedidaPage() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    getUnidadesDeMedida({ page, per_page: perPage })
      .then(({ data }) => {
        if (data.meta) {
          setItems(data.data ?? [])
          setMeta(data.meta)
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteUnidadDeMedida(deleting.id)
      setDeleting(null)
      load()
    } catch {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Catálogo de Unidades de Medida</h2>
          <p className="text-sm text-gray-500 mt-0.5">
            {(meta?.total ?? items.length)} unidad{(meta?.total ?? items.length) !== 1 ? 'es' : ''} registrada{(meta?.total ?? items.length) !== 1 ? 's' : ''}
          </p>
        </div>
        {isSuperAdmin && (
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            <Ruler size={40} className="mx-auto text-gray-200 mb-3" />
            <p className="text-gray-400 text-sm">
              {search ? 'Sin resultados para tu búsqueda' : 'Aún no hay unidades de medida registradas'}
            </p>
            {!search && isSuperAdmin && (
              <Button
                variant="secondary"
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map((u) => (
                  <tr key={u.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 text-gray-700">{u.descripcion}</td>
