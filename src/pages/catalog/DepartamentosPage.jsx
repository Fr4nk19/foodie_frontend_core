...
import { Plus, MapPin, Search, RefreshCw, Pencil, Trash2 } from 'lucide-react'
import {
  getDepartamentos,
  createDepartamento,
  updateDepartamento,
...
// ─── Form (create / edit) ─────────────────────────────────────────────────────
function DepartamentoForm({ initial, onSuccess, onCancel }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (isEdit) {
        await updateDepartamento(initial.id, data)
      } else {
      ...
      if (fieldErrors) {
        ...
        setApiError(first ?? msg ?? 'Error al guardar el departamento.')
      } else {
        setApiError(msg ?? 'Error al guardar el departamento.')
      }
    }
  }
...
export default function DepartamentosPage() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    getDepartamentos({ page, per_page: perPage })
      .then(({ data }) => {
        if (data.meta) {
          setItems(data.data ?? [])
          setMeta(data.meta)
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDepartamento(deleting.id)
      setDeleting(null)
      load()
    } catch {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Catálogo de Departamentos</h2>
          <p className="text-sm text-gray-500 mt-0.5">
            {(meta?.total ?? items.length)} departamento{(meta?.total ?? items.length) !== 1 ? 's' : ''} registrado{(meta?.total ?? items.length) !== 1 ? 's' : ''}
          </p>
        </div>
        {isSuperAdmin && (
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-gray-400 text-sm">
              {search ? 'Sin resultados para tu búsqueda' : 'Aún no hay departamentos registrados'}
            </p>
            {!search && isSuperAdmin && (
              <Button
                variant="secondary"
