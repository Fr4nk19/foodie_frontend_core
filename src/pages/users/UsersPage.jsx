...
// ─── User Form ────────────────────────────────────────────────────────────────
function UserForm({ initial, companies, onSuccess, onCancel }) {
  ...
  const {
    ...
    defaultValues: initial
      ? { name: initial.name, email: initial.email, role: initial.role,
          company_id: initial.company?.id ?? '', is_active: initial.is_active }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {/* Empresa */}
        <div className="flex flex-col gap-1">
          <label htmlFor="company_id" className="text-sm font-medium text-gray-700">Empresa</label>
          <select
            id="company_id"
            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
            {...register('company_id')}
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-gray-400 text-sm">
              {search ? 'Sin resultados para tu búsqueda' : 'Aún no hay usuarios registrados'}
            </p>
            {!search && (
                ...
                {filtered.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4 text-gray-500">{u.company?.name ?? '—'}</td>
                      <td className="px-6 py-4">
